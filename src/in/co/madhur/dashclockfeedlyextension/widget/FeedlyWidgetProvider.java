...
public abstract class FeedlyWidgetProvider extends AppWidgetProvider {
		...
		for (int i = 0; i < N; ++i) {
			try {
				...
				appWidgetManager.notifyAppWidgetViewDataChanged(
						appWidgetIds[i], R.id.widgetListView);
	...
	private RemoteViews updateWidgetListView(Context context, int appWidgetId) {
		...
		if (!appPreferences.IsTokenPresent()) {
			remoteViews.setTextViewText(R.id.empty_view,
					context.getString(R.string.login_required_desc));
			...
			remoteViews.setTextViewText(R.id.updatedTextView, String.format(context
					.getString(R.string.lastupdate_display_format), Utils
					.GetFormattedDate(appPreferences.GetLastSuccessfulSync(),
							context)));

			if (appPreferences.GetWidgetIntent() != null)
				remoteViews.setOnClickFillInIntent(R.id.widgetListView,
						appPreferences.GetWidgetIntent());
	...
	private void SetupLookAndFeel(Context context, RemoteViews remoteViews,
			AppPreferences appPreferences) {

		remoteViews.setInt(R.id.widget_host, "setBackgroundColor",
				appPreferences.GetColor(Keys.WIDGET_BACKGROUND_COLOR));
		...
		if (appPreferences.IsWidgetHeaderEnabled()) {
			...
			remoteViews.setOnClickPendingIntent(R.id.widget_refresh,
					new Alarms(context).GetPendingIntent(context));
			remoteViews.setOnClickPendingIntent(R.id.widget_settings,
					GetSettingsPendingIntent(context));
		...
		remoteViews.setTextColor(R.id.CountTextView,
				appPreferences.GetColor(Keys.WIDGET_COUNT_COLOR));

		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
			remoteViews.setTextViewTextSize(R.id.updatedTextView,
					TypedValue.COMPLEX_UNIT_SP, appPreferences.GetFontSize());
		} else {
			remoteViews.setFloat(R.id.updatedTextView, "setTextSize",
